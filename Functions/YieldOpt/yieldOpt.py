...
TRANSACTION_COST = 0.00000002  # Cost for each reallocation
...
# ---------------------------
#   APY DATA (Replace with Live Data)
# ---------------------------
APY_DATA = {
    "Curve": 0.02,  # 2%
    "Aave": 0.03,   # 3%
    "Lido": 0.08    # 8% (higher difference to encourage movement)
}
...
# ---------------------------
def get_apy():
    """ Returns APYs from DeFi protocols """
...
def portfolio_value(state, apys):
    ...
    valA, valB, valC = (a * UNIT_SIZE, b * UNIT_SIZE, c * UNIT_SIZE)
    gainA, gainB, gainC = (valA * apyA, valB * apyB, valC * apyC)
...
def get_possible_actions(state):
    ...
    a, b, c = state
    if a > 0:
        actions.append(("Curve->Aave", 1))
        actions.append(("Curve->Lido", 1))
    if b > 0:
        actions.append(("Aave->Curve", 1))
        actions.append(("Aave->Lido", 1))
    if c > 0:
        actions.append(("Lido->Curve", 1))
        actions.append(("Lido->Aave", 1))
...
def apply_action(state, action):
    ...
    if direction == "Curve->Aave" and a >= amt:
        return (a - amt, b + amt, c)
    elif direction == "Curve->Lido" and a >= amt:
        return (a - amt, b, c + amt)
    elif direction == "Aave->Curve" and b >= amt:
        return (a + amt, b - amt, c)
    elif direction == "Aave->Lido" and b >= amt:
        return (a, b - amt, c + amt)
    elif direction == "Lido->Curve" and c >= amt:
        return (a + amt, b, c - amt)
    elif direction == "Lido->Aave" and c >= amt:
        return (a, b + amt, c - amt)
...
def parse_json_to_state(data):
    ...
    for asset in data["lido"]:
        balance = float(asset["balance"])
        price = float(asset["quote"])
        protocol_values["Lido"] += (balance / 1e9) * price  # Scale down stETH
    ...
    state = tuple(int((v / total_value) * 10) for v in protocol_values.values())
...
def format_action(action):
    """ Converts action to user-friendly message. """
    direction, amount = action
    if direction == "NOOP":
        return "No rebalancing needed."
    return f"Rebalance: Move {amount * UNIT_SIZE} units from {direction.split('->')[0]} to {direction.split('->')[1]}."
...
def yeild_optimize(json):
    print(json)
    json_data = {
        "curve": [
            {"chain_name": "eth-mainnet", "balance": "35.20781919682791", "contract_name": "USD Coin", "contract_ticker_symbol": "USDC", "quote_currency": "USD", "quote": 1.0},
    ...
    formatted_action = format_action(best_act)
